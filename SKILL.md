...
3. **Translate styling → tokens.** Colors → `primary/secondary/accent/muted/foreground/background/border` tokens; radius → `rounded-sm|md|lg|xl` (these follow the owner's theme; `rounded-full` stays a pill on every site, so use it only where the SHAPE is the point); fonts → `var(--font-heading)`; drop external CSS/fonts/icon libraries (inline SVG instead). Keep a `<style>` block only for keyframes/masks/pseudo-elements.
...
6. **Check.** Run `npx @webto-id/variant-check <file>.astro --type <type> --strict --content sample.json --out preview.html`. Write `variant.json` (step 8) BEFORE this run and keep it in the same folder: the CLI picks it up automatically and checks `mood`/`description`/`sectionType` against the platform's own enum, which is what the upload rejects on. Fix every `✖` and `▲` (strict turns quality warnings into errors — that is the marketplace's submit gate). Open `preview.html`; try `--theme dark`.
