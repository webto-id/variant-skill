...
When an element's size is part of the design and must not be rescaled (a member name that must stay small, a stat number that must not blow out of its card): use `!text-base` (any `!text-*`), including behind a responsive variant — `sm:!text-2xl`, `md:!mt-4` (compiler ≥ 0.1.7; earlier versions dropped the variant-prefixed form as `tailwind-skipped` while the bare `!text-*` form worked, which was a compiler bug, not a rule — a responsive `!important` override is the documented way to give a heading its own scale across breakpoints). The `!` prefix is **honored everywhere** - the platform never rewrites it, neither the theme's heading scale nor a site owner's per-section body-size override touches it. It stays rem-based, so it only follows the site-wide base body size; an inline `style="font-size: ..."` pins a size absolutely.
