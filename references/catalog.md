...
- **`mood`** — 1 to 4 values from this CLOSED enum, never invent one: `editorial` `luxurious` `airy` `minimal` `technical` `precise` `corporate` `playful` `warm` `casual` `bold` `brutal` `calm` `dense` `dark`. Map the design's feel to the NEAREST listed mood ("elegant" → `luxurious`, "romantic" → `warm`, "fun" → `playful`, "structured" → `precise`, "direct" → `bold`).
