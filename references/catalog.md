...
- **`mood`** — 1 to 4 values from this CLOSED enum, never invent one: `editorial` `luxurious` `airy` `minimal` `technical` `precise` `corporate` `playful` `warm` `casual` `bold` `brutal` `calm` `dense` `dark`. From `variant-check` 0.1.29 the CLI reads `variant.json`/`template.json` next to the `.astro` (or one level up) and checks this enum itself — an invented mood is an error locally instead of a rejected upload, and `--strict` also fails an empty `mood`/`description`, matching the submit gate. Map the design's feel to the NEAREST listed mood ("elegant" → `luxurious`, "romantic" → `warm`, "fun" → `playful`, "structured" → `precise`, "direct" → `bold`).
- **Energy vs composition — the distinction the enum is easiest to get wrong on.** `bold` / `brutal` / `playful` are the loud end and `calm` is the quiet one; `airy` (lots of empty space) and `minimal` (few elements) say nothing about energy, and a section can be spacious and still shout — a 96px headline in an accent colour on white is `bold` AND `airy`. Reach for `calm` when the pace is slow and the contrast low: no typographic shouting, muted colour, nothing demanding attention first. Do NOT use it for "simple" (`minimal`), "lots of room" (`airy`), "warm" (`warm`) or "expensive-looking" (`luxurious`), or it becomes the next overflow bucket.
- **Mood describes the LOOK, never the subject.** A one-person portfolio is not a mood — that belongs in `description` and `fits`. A value that can sit on both a `brutal` and a `luxurious` variant tells the wizard nothing.
