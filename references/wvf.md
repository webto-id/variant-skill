...
- **The owner's page-navigation switches reach you ONLY through these props.** Page Settings > Navigation has "show in navbar" and "show in footer" per page; a navbar that builds its links from `links[]` alone, or a footer that never reads `footerPages`, ignores both switches and the owner has no way to tell why. Render `pages` in a navbar and `footerPages` in a footer, both as-is — the filtering is already done. `links[]` is for the EXTRA links a design wants (anchors, social, external), never a replacement for the site's own pages. Lints: `chrome-nav-pages-missing` / `chrome-footer-pages-missing` (warnings, `variant-check` 0.1.31).
- **Context props are injected by the renderer, never edited by the owner.** Declare them in `interface Props` to read them; the compiler keeps them out of the editor schema: `pages?: Array<{ title: string; slug: string }>` (the site's nav pages, filtered by the owner's per-page "show in navbar" switch — a one-page site gets `[]`), `currentSlug?: string` (for active-link styling), `linkPrefix?: string` (path prefix to PREPEND to every internal href: `url(linkPrefix + (p.slug ? "/" + p.slug : "/"))`; today it is always `""` for uploaded chrome — emit plain site-relative paths and NEVER build query strings yourself: preview/dev params (`?_subdomain`, `_pt`, ...) are re-attached to every link by a platform script), `colorMode?: "light"|"dark"|"system"`; footers additionally get `footerPages?: Array<{ title: string; slug: string }>` (only the pages the owner ticked for the footer; each item also carries `label`/`url`, the spelling the platform's own footers read) (photo/icon attribution is rendered by the PLATFORM in a strip below the footer — a footer variant never needs to render credits itself). `siteName` is an ORDINARY editable base field — the renderer merely injects a fallback that `content.siteName` overrides. Everything else you declare is ordinary content.
...
| `content-type` `content-required` `content-unknown-key` `content-min-items` `content-max-items` `content-enum` `content-invalid` `content-image-url` | error | `--content` validated against the base schema (§9) — always an error, `--strict` or not |
...
```
npx @webto-id/variant-check <file.astro> --type <sectionType> [--strict] [--content sample.json] [--manifest variant.json] [--out preview.html] [--theme light|dark|warm] [--json]
```
...
**Image fields have a shape, enforced from 0.1.30.** Every base image field (`logos[].image`, `members[].image`, `features[].image`, `products[].image`, `steps[].image`, `posts[].image`, `beforeImage`/`afterImage`, `logoUrl`/`logoUrlDark`) accepts only: an absolute `http(s)` URL, an uploaded path (`/r2/`, `/media/`, `/illu/`), one of the sentinels `__IMG__:` / `__ILLU__:` / `asset:`, or empty. A relative path such as `assets/logo.png` is rejected by the upload — and used to lint clean here, because the platform rule is a Zod `.refine()` and JSON Schema cannot carry one. Those fields now ship tagged, so `--content` reports `content-image-url` instead of letting the upload find it.

**The manifest is checked too, from 0.1.29.** `<key>.variant.json` / `variant.json` (or a bundle's `template.json`) is picked up automatically when it sits beside the `.astro` or one level up -- so `sections/<key>.astro` finds its bundle manifest with no flag; `--manifest <file>` points at it explicitly, `--no-manifest` opts out. The rules are generated from the platform's own schema, so the `mood` enum here IS the enum the upload enforces. **Error**: a mood outside the enum, more than 4 of them, a `description` under 15 characters, an unknown `sectionType`, a bad or duplicated `key` (a standalone `variant.json` needs none -- the uploader takes it from the filename, and a key that disagrees only warns), more than 12 variants in one `template.json`, or a `sectionType` that disagrees with `--type`. **Warning**: `description`/`mood` absent (legal for a draft and for a patch-mode re-upload, refused at submit -- so `--strict` makes both errors), a field name the upload would silently strip (`moods`, `tags`), a legacy `hero-*` alias, and an `.astro` with no matching entry. Before this, a bundle could lint clean and still be rejected for four invented moods.
