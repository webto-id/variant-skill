...
**The manifest is checked too, from 0.1.29.** `variant.json` (or a bundle's `template.json`) is picked up automatically when it sits beside the `.astro` or one level up -- so `sections/<key>.astro` finds its bundle manifest with no flag; `--manifest <file>` points at it explicitly, `--no-manifest` opts out. The rules are generated from the platform's own schema, so the `mood` enum here IS the enum the upload enforces. **Error**: a mood outside the enum, more than 4 of them, a `description` under 15 characters, an unknown `sectionType`, a bad or duplicated `key`, more than 12 variants in one `template.json`, or a `sectionType` that disagrees with `--type`. **Warning**: `description`/`mood` absent (legal for a draft and for a patch-mode re-upload, refused at submit -- so `--strict` makes both errors), a field name the upload would silently strip (`moods`, `tags`), a legacy `hero-*` alias, and an `.astro` with no matching entry. Before this, a bundle could lint clean and still be rejected for four invented moods.
